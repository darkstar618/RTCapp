...
// Serialize writes to avoid SQLite SQLITE_BUSY under concurrent load
let writeQueue = Promise.resolve();
function queued(fn) { writeQueue = writeQueue.then(fn).catch(fn); return writeQueue; }
const stmts = {
...
function getChannel(id) { return stmts.getChannel.get(id) || null; }
function closeChannel(id) { stmts.closeChannel.run(Date.now(), id); return getChannel(id); }
...
function listParticipants(channelId) { return stmts.listParticipants.all(channelId); }
function serializeChannel(ch) {
  const count = stmts.countParticipants.get(ch.id);
  return { id: ch.id, app_id: ch.app_id, created_at: ch.created_at, closed_at: ch.closed_at || null, participant_count: count ? count.count : 0 };
}
