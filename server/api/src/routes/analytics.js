...
router.get('/overview', authenticateDeveloper, (req, res) => {
...
router.get('/channels/daily', authenticateDeveloper, (req, res) => {
  const appId = req.dev.app_id;
  if (!appId) return res.status(400).json({ error: 'app_id claim required on developer token' });
  const days = 14;
  const result = [];
...
router.get('/sessions/daily', authenticateDeveloper, (req, res) => {
  const appId = req.dev.app_id;
  if (!appId) return res.status(400).json({ error: 'app_id claim required on developer token' });
  const days = 14;
  const result = [];
...
router.get('/peak-hours', authenticateDeveloper, (req, res) => {
  const appId = req.dev.app_id;
  if (!appId) return res.status(400).json({ error: 'app_id claim required on developer token' });
  const since = Date.now() - 30 * 24 * 60 * 60 * 1000;
...
router.get('/top-channels', authenticateDeveloper, (req, res) => {
  const appId = req.dev.app_id;
  if (!appId) return res.status(400).json({ error: 'app_id claim required on developer token' });
  const since = Date.now() - 30 * 24 * 60 * 60 * 1000;
  ...
  res.json(rows.map((r) => ({
    ...
    total_minutes: r.total_ms ? Math.round(r.total_ms / 60000) : 0,
  })));
});
